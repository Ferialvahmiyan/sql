...
select v.vendor_name, vba.vendor_id, count(vba.vendor_id) as total_rented
...
select customer_first_name, customer_last_name, SUM(quantity*cost_to_customer_per_qty) as total_cost
...
select customer_first_name, customer_last_name,  strftime('%m', market_date) as month, strftime('%Y',market_date) as year, 
SUM(quantity*cost_to_customer_per_qty) as total_cost
from customer_purchases as cp
join customer c
on c.customer_id=cp.customer_id
where month = '04' and year = '2022'
group by customer_first_name, customer_last_name, year, month
